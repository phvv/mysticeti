...
fn direct_commit() {
    ...
    for number_of_leaders in 1..committee.len() {
        ...
        build_dag(&committee, &mut block_writer, None, wave_length + 1);
        ...
        let threshold_round = wave_length + 1;
...
fn multiple_direct_commit() {
    ...
    for n in 1..=10 {
        let enough_blocks = wave_length + n;
        let mut block_writer = TestBlockWriter::new(&committee);
...
fn direct_commit_partial_round() {
    ...
    let enough_blocks = 2 * wave_length;
    let mut block_writer = TestBlockWriter::new(&committee);
    build_dag(&committee, &mut block_writer, None, enough_blocks);
...
fn indirect_commit() {
    ...
    let references_without_leader_1: Vec<_> = references_1
        ...
        .collect();
    tracing::info!("number_of_leaders: {:?}", number_of_leaders);
    tracing::info!(
        "references_without_leader_1: {:?}",
        references_without_leader_1
    );

    // Only 2f+1 validators support that leader.
    ...
    let sequence = committer.try_commit(last_committed, threshold_round);
    tracing::info!("Commit sequence: {sequence:?}");
    assert_eq!(sequence.len(), 2 * wave_length as usize * number_of_leaders);
...
fn indirect_skip() {
    ...
    // Only f+1 validators support that leader.
    let mut references = Vec::new();
    ...
    let decision_round_3 = 5 + wave_length;
    build_dag(
        &committee,
        &mut block_writer,
        Some(references),
...
fn undecided() {
    ...
    // Filter out the first leader of round 1.
    ...
    // Create a dag layer where only one authority supports that leader.
    ...
    // Add enough blocks to reach the decision round of round 1.
    let decision_round_1 = wave_length + 1;
    build_dag(
        &committee,
        &mut block_writer,
