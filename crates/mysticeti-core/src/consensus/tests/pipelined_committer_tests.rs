...
fn indirect_commit() {
    ...
    // Due to the pipeline, there is a leader per round. The 5th leader is decided in the final
    // round of its wave.
...
fn indirect_skip() {
    ...
    // Only 1 validator supports the 4th leader.
    ...
    // Filter out the authority which supported the 4th leader.
    let references_without_booster_4: Vec<_> = references_5
        .iter()
        .cloned()
        .filter(|x| x.authority != committee.authorities().next().unwrap())
        .collect();

    // f+1 validators support the 4th leader in the next round (the decision round).
    let mut references_6 = Vec::new();

    let connections_with_booster_4 = committee
        .authorities()
        .take(committee.validity_threshold() as usize)
        .map(|authority| (authority, references_5.clone()))
        .collect();
    references_6.extend(build_dag_layer(
        connections_with_booster_4,
        &mut block_writer,
    ));

    let connections_without_booster_4 = committee
        .authorities()
        .skip(committee.validity_threshold() as usize)
        .map(|authority| (authority, references_without_booster_4.clone()))
        .collect();
    references_6.extend(build_dag_layer(
        connections_without_booster_4,
        &mut block_writer,
    ));

    // Add enough blocks to reach the decision round of the 7th leader.
    // Due to the pipeline, there is a leader per round. The 7th leader is decided in the final
    // round of its wave.
    let decision_round_7 = wave_length + 6;
    build_dag(
        &committee,
        &mut block_writer,
        Some(references_6),
